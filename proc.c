...
wait(void)
{
  ...
  for(;;){
    ...
    if(!foundChildThreads || curproc->killed){
      release(&ptable.lock);
      return -1;
    }
...
clone(void(*fcn)(void*,void*), void *arg1, void *arg2, void* stack)
{
  ...
  *np->tf = *p->tf;  // Copy the trap frame of the parent process to the new thread's trap frame
  ...
  *(uint*)stackReturnAddress = 0xFFFFFFF; // fake return PC
...
int join(void** stack)
{
  ...
  for (;;) { // Loop forever
    ...
    // Scan through the process table looking for child threads of the calling process
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
      ...
      // If this is a child thread, set the flag to indicate that we have children
      foundChildThreads = 1;
      ...
      if (p->state == ZOMBIE) {
        ...
        // Release the process table lock and return the process ID of the zombie child thread
        release(&ptable.lock);
        return pid;
      }
    }

    // If there are no children or the calling process has been killed, release the process table lock and return an error code
    if (!foundChildThreads || cp->killed) {
      release(&ptable.lock);
      return -1;
    }

    // If there are still child threads running, go to sleep until one of them exits
